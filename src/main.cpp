...
int main(int argc, char* argv[]) {
    ...
    std::future<mongocxx::client> mongoFuture = std::async(std::launch::async, []() {
        ...
        QObject::connect(&w, &MainWindow::buttonPushed, [&w, &stockCollection, &optionCollection, &errorBuffer]() {
            try {
                ...
                yahoo::stockprice::downloadCSVtoCloud(w.getTicker().toStdString(), date1, date2, w.getInterval(), stockCollection, errorBuffer);
                yahoo::options::downloadOptionsToCloud(w.getTicker().toStdString(), w.getExpirationDate().toStdString(), optionCollection);
                QString errorQstring = QString::fromStdString(errorBuffer);
